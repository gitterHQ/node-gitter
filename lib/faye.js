...
var SnapshotExt = function(emitter) {
...
SnapshotExt.prototype.incoming = function(message, callback) {
  if(message.channel == '/meta/subscribe' && message.ext && message.ext.snapshot) {
    ...
    this.emitter.emit('snapshot', message.ext.snapshot);
  }
...
var FayeClient = function(token, opts) {
  ...
  this.client.addExtension(new SnapshotExt(this.emitter));
};

//['chatMessages', 'events', 'users'].forEach(function(resource) {
//  FayeClient.prototype[resource] = function(roomId) {
//    var self = this;
//
//    this.subscriptions[resource] = this.client.subscribe('/api/v1/rooms/' + roomId + '/' + resource, function(msg) {
...
FayeClient.prototype.subscribeTo = function(resource, eventName) {
  ...
  console.log('subcribing to:', resource);

  this.subscriptions[resource] = this.client.subscribe(resource, function(msg) {
    self.emitter.emit(eventName, msg);
  });

  return this.emitter;
};

FayeClient.prototype.events = function() {
  return this.emitter;
};
