...
var Rooms = function(client, users, opts) {
  this.opts   = opts;
  this.client = client;
  this._users = users;
  this.path   = '/rooms';
...
Rooms.prototype.join = function(room_uri, cb) {
  ...
  .then(function(res) {
    return cb ? cb(null, self.extend(res)) : self.extend(res);
  });
};
...
Rooms.prototype.extend = function(room) {
  ...
  ['users', 'channels', 'chatMessages'].forEach(function(resource) {
    room[resource] = function(query, cb) {
      return self[resource](room.id, query).nodeify(cb);
    };
  });

  room.send = function(message, cb) {
    return self.sendMsg(room.id, message).nodeify(cb);
  };
  ...
  room.streaming = function() {
    if (!this._fayeClient) {
      ...
      opts.roomId = room.id;
      if (self.opts.faye) {
        opts.host = self.opts.faye.host;
        opts.clientType = self.opts.faye.clientType;
      }
      this._fayeClient = new FayeClient(self.client.token, opts);
    }
